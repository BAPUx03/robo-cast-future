          ...
          <dl className="reveal-on-scroll grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-border bg-border sm:grid-cols-3 lg:grid-cols-6">
            {automationStats.map((s) => (
              <div key={s.label} className="bg-card px-4 py-7 text-center">
                <dd className="font-display text-3xl font-bold leading-none text-gradient-brand">
                  {s.value}
                </dd>
                ...
                <div className="p-7 sm:p-9">
                  ...
                  <div className="mt-6 flex flex-wrap gap-2">
                    {s.applications.map((application) => (
                      <span
                        key={application}
                        className="rounded-full border border-border bg-secondary/40 px-3 py-1 font-mono text-[9px] uppercase tracking-wider text-muted-foreground"
                      >
                        {application}
                      </span>
                    ))}
                  ...
                  <div className="border-t border-border bg-carbon-2/40 p-5 sm:p-7 lg:col-span-2">
                    ...
                    >
                      {s.gallery.map((g, gi) => (
                        <div
                          key={gi}
                          className="overflow-hidden rounded-xl border border-border bg-carbon-2"
                        >
                          <img
                            src={g}
                            alt={`${s.title} installation ${gi + 1}`}
                            loading="lazy"
          ...
          <div className="reveal-on-scroll rounded-2xl border border-border bg-card/60 px-6 py-8 sm:px-10">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {automationIndustries.map((i) => (
                <span
                  key={i}
                  className="rounded-full border border-border bg-background px-4 py-1.5 font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground"
                >
                  {i}
                </span>
              ))}
          ...
          <div className="reveal-on-scroll rounded-2xl border border-border bg-card/60 px-6 py-8 sm:px-10">
            ...
            <div className="mt-7 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
              {automationPartners.map((partner) => (
                <div
                  key={partner.name}
                  className="grid min-h-24 place-items-center rounded-xl border border-border bg-background p-5"
                >
                  <img
                    src={partner.image}
                    alt={partner.name}
