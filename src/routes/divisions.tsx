...
export const Route = createFileRoute("/divisions")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Two engineering divisions under one roof: Robotics & Automation, and Investment Casting. Learn what each delivers." },
      { property: "og:title", content: "Modtech Divisions — Robotics & Casting" },
      { property: "og:description", content: "Robotics & Automation and Investment Casting — explained side by side." },
    ],
  }),
              ...
              >
                <div className="relative aspect-[16/10] overflow-hidden">
                  ...
                  <div className="absolute left-4 top-4 flex items-center gap-2">
                    ...
                    <span className="cat-tag rounded-md border border-border bg-card/80 px-2.5 py-1 font-mono text-[10px] uppercase tracking-widest text-muted-foreground backdrop-blur transition">{categoryMeta[d.category].label}</span>
                  </div>
                </div>
                <div className="p-7 sm:p-8">
                  ...
                  <ul className="mt-6 grid grid-cols-2 gap-2">
                    {d.points.map((p) => (
                      <li key={p} className="flex items-center gap-2 rounded-md border border-border/70 bg-secondary/40 px-3 py-2 font-mono text-[11px] uppercase tracking-wider text-muted-foreground">
                  ...
                  <div className="mt-7 flex flex-wrap items-center gap-3">
                    <Link to="/machines" className="inline-flex items-center gap-2 rounded-full bg-brand px-5 py-2.5 font-mono text-[11px] font-semibold uppercase tracking-wider text-brand-foreground shadow-glow transition hover:translate-y-[-2px]">
                      See machines <ArrowRight className="h-3.5 w-3.5" />
                    </Link>
